...
with open("db/seed.sql", "w") as f:
    ...
    for index, row in items.iterrows():
        brand = brand_dict[row['brand']]
        title = str(row['title']).replace("'", "''")
        f.write(f"INSERT INTO Cellphone VALUES ({index+1}, '{row['asin']}', {brand}, '{title}', '{row['url']}', '{row['image']}', {row['rating']}, '{row['reviewUrl']}', {row['totalRatings']}, {row['price']}, {row['originalPrice']});\n")
    ...
    for index, row in reviews.iterrows():
        ...
        cellphone_id = items['asin'].eq(row['asin']).idxmax()
